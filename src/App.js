...
function App(props) {
  ...
  useEffect(() => {
    ...
    if (storedArray) {
      setListPeople(storedArray);
    }
  }, []);
  useEffect(() => {
    if (listPeople.length > 0) {
      localStorage.setItem('myArray', JSON.stringify(listPeople));
    }
  }, [listPeople]);

  // הוספת איבר למערך לדוגמה
  const addPeopleToList = (newPeople) => {
    let copy = [...listPeople];
    newPeople={...newPeople,dateDonate:new Date()}
     copy.push(newPeople);
      setListPeople(copy); //העתקה עמוקה
      ...
      <CoinContext.Provider value={{ coin: coin, changeCurrencyType: changeCurrencyType }}>
        ...
        <div className="app-container" dir='rtl'>
          <ThemeProvider theme={styleMode == "day" ? myStyleDay : myStyleNight}>
            <Main listPeople={listPeople} coin={coin}/>
            <Routes>
